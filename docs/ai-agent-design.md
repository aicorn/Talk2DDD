...
## 20. 待澄清问题管理机制（ClarificationManagement）

### 20.1 概念定义

**待澄清问题（Clarification Question）** 是在 DDD 对话过程中识别到的信息缺口或歧义点，需要用户明确回答后才能继续推进领域建模。它们由 LLM 在对话中提出，通过 XML 标记注入到 `clarification_queue` 队列，并在阶段文档中可见。

待澄清问题具有以下生命周期：

```
[新增] → 出现在阶段文档的「待澄清问题」区块
    ↓
用户回答 / 用户主动忽略
    ↓
[删除] → 从队列中彻底移除（不保留勾选记录）
...
**核心规则**：
- 已解决或忽略的问题从队列中**删除**，阶段文档不保留历史记录
- 只要队列非空，**禁止**提示用户进入下一阶段
- 用户点击「下一阶段」按钮时，若队列非空，系统**拦截**切换并要求用户先处理问题

### 20.2 数据结构

```
ClarificationQuestion
├── id: str          # 自动生成，如 Q001、Q002
├── question: str    # 待澄清的问题文本
├── scenario_id: str | None   # 关联的业务场景 ID（可选）
└── answered: bool   # 内部字段（兼容旧数据），新流程以删除为主
```

`AgentContext.clarification_queue: List[ClarificationQuestion]` 始终只包含**尚未解决**的问题。

### 20.3 XML 标记规范（LLM 输出格式）

| 操作 | XML 格式 | 效果 |
|------|---------|------|
| 新增待澄清问题 | `<clarification id="Q001">问题文本</clarification>` | 追加到队列 |
| 用户已回答（解决） | `<clarification id="Q001" answered="true"/>` | 从队列中删除 |
| 用户明确忽略（跳过） | `<clarification id="Q001" dismissed="true"/>` | 从队列中删除 |

**去重逻辑**：相同 ID 或相同问题文本不会重复加入队列。

### 20.4 LLM 行为准则

1. **提出问题**：当对话中发现信息缺口（如业务规则不明确、场景边界模糊）时，通过 `<clarification>` 标记提出问题，并在对话中向用户说明。
2. **解决问题**：用户回答后，LLM 识别对应问题已得到答案，立即输出 `<clarification id="Qxxx" answered="true"/>` 将其从队列移除。
3. **忽略问题**：若用户明确表示「先不考虑」、「跳过这个」、「暂时忽略」等，LLM 输出 `<clarification id="Qxxx" dismissed="true"/>` 移除该问题。
4. **优先级**：若 CONTEXT_BLOCK 中存在待澄清问题，LLM 必须优先引导用户逐一处理，**不得**提示进入下一阶段。
5. **问题数量**：每次最多提出 1~2 个新的待澄清问题，避免一次性抛出过多问题让用户不知所措。

### 20.5 阶段文档渲染规则

- **所有阶段文档**（P1~P5）在队列非空时，末尾均追加「待澄清问题」区块
- 区块格式为复选框列表（`- [ ] Qxxx：问题文本`）
- 每次对话后自动重新渲染，已删除的问题不再出现
- P2 业务需求草稿示例：
...
当用户点击「下一阶段 →」按钮时（`POST /api/v1/agent/switch-phase` direction="next"）：

```
① 加载 AgentContext
② 检查 clarification_queue 是否为空
   ├── 非空 → 拦截切换：
   │           • 不执行 advance_phase()
   │           • 调用 AI 生成提示消息，要求用户先解决待澄清问题
   │           • 返回 AgentResponse（phase_changed=false）
   └── 空   → 正常切换到下一阶段
```

拦截时 AI 接收到的系统触发消息格式：
```
[系统] 用户尝试进入下一阶段，但仍有 N 个待澄清问题未解决。
请明确提示用户必须先处理这些问题，再才能进入下一阶段。
当前待澄清问题列表：
- [Q001] 问题文本
- [Q002] 问题文本
```
...
| 维度 | 待澄清问题 | 需求变更 |
|------|-----------|---------|
| 发起方 | LLM（识别信息缺口时） | LLM（识别用户变更意图时） |
| 数据结构 | `clarification_queue`（瞬态队列） | `requirement_changes`（永久历史） |
| 生命周期 | 解决后删除 | 永久保留历史记录 |
| 对阶段切换的影响 | 非空时拦截"下一阶段" | 触发阶段回溯（P3+→P2） |
| 在阶段文档中的体现 | 当前待解决列表 | 变更历史表格（P5 修订记录） |
