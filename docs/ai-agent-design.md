...
- **快捷跳转**：用户可在任意阶段输入指令（`/generate`, `/model`）直接跳转到目标阶段。
...
```json
{
  ...
  "description": "记录用户确认（或跳过）的技术栈偏好，供 TECH_ARCHITECTURE 文档生成使用",
  "parameters": {
    "type": "object",
    "properties": {
      "skipped": {
...
| `TECH_ARCHITECTURE` | 基于领域模型 **和用户已确认的技术栈偏好** 推荐技术边界和分层；若用户跳过技术栈确认，则由 AI 根据领域模型自动推荐并说明理由 | 架构建议 Markdown |
...
| M13 | 前端 STALE 文档提示 + 需求变更时间轴 | 🟢 低 |
| M14 | 斜线命令解析（含 `/change`, `/changes`, `/regenerate`, `/techstack`） | 🟢 低 |
...
> **核心目标**：在 P4 模型设计阶段完成后，通过自然对话的方式采集用户的技术栈偏好，让 AI 生成的技术架构文档真正贴合用户团队的能力边界。

---

### 17.2 确认时机与流程

技术栈确认嵌入在 **P4 → P5 的过渡阶段**，在领域模型草稿经用户确认后自然触发。
...
`TECH_ARCHITECTURE` 文档的 System Prompt 模板需注入技术栈上下文：
...
约束说明：
  - 标注 "用户指定" 的项必须采用，不可替换。
  - 标注 "AI 推荐" 的项若有更优方案可建议，但需在文档中说明。
  - 用户熟悉程度为 LEARNING 或 UNFAMILIAR 的技术需在文档中补充学习资源链接或替代方案。
[/TECH_STACK_BLOCK]
```

**生成策略**：

| `skipped` | `confirmed` | 生成策略 |
|-----------|-------------|---------|
| `false` | `true` | 严格遵循用户指定的技术栈，AI 仅在未指定的分类中推荐 |
| `true` | `true` | AI 根据领域模型全自动推荐，并在文档中说明每项选型的原因 |
| `false` | `false` | 技术栈尚未采集，生成通用架构建议并提示用户补充偏好 |

---

### 17.6 技术栈变更处理

用户在 P5/P6 阶段可通过 `/techstack` 命令或直接在对话中说明重新调整技术栈。变更处理方式与需求变更类似：

1. Agent 更新 `tech_stack_preferences` 中对应分类的 `TechChoice` 列表。
2. 将已生成的 `TECH_ARCHITECTURE` 文档标记为 `STALE`，提示用户重新生成。
3. 不影响其他类型的文档（`DOMAIN_MODEL`、`USE_CASES` 等与技术栈无关）。

```
用户: 我们决定把数据库换成 PostgreSQL，不用 MySQL 了。

AI:   已更新数据库偏好为 PostgreSQL。
      由于技术栈变更，之前生成的「技术架构建议」文档已标记为
      ⚠️ 待更新。输入 /regenerate TECH_ARCHITECTURE 重新生成。
```
...
- **`/techstack` 快捷入口** — 在 P5/P6 阶段的阶段文档面板底部提供"修改技术栈偏好"链接，点击触发 `/techstack` 命令。
...
| 方案 | 说明 | 结论 |
|------|------|------|
| 强制确认 | 不完成技术栈确认就无法进入 P5 | ❌ 增加用户摩擦；部分用户只需要领域模型文档，与架构无关 |
