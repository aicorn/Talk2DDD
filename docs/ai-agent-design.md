...
| 阶段 | 标识 | 名称 | 目标 | 退出条件 |
|------|------|------|------|----------|
| 1 | `ICEBREAK` | 破冰引入 | 了解用户角色和项目背景 | 收集到项目名称、领域背景 |
| 2 | `REQUIREMENT` | 需求收集 | 梳理业务流程和痛点 | 收集到 ≥3 个核心业务场景 |
| 3 | `DOMAIN_EXPLORE` | 领域探索 | 识别核心概念和术语 | 识别出 ≥5 个领域概念候选 |
| 4 | `MODEL_DESIGN` | 模型设计 | 设计聚合、实体、值对象 | 用户确认领域模型草稿 |
| 5 | `DOC_GENERATE` | 文档生成 | 生成各类 DDD 文档 | 至少一类文档生成完成 |
| 6 | `REVIEW_REFINE` | 审阅完善 | 根据反馈修订文档 | 用户标记项目完成 |
...
- **STALE 文档提示** — 当 `stale_documents` 非空时，在文档列表旁显示橙色"需更新"徽标，并提示用户重新生成。
...
| M3 | KnowledgeExtractor（基于 XML 标记解析） | 🟡 中 |
| M4 | ToolDispatcher + Function Calling 集成 | 🟡 中 |
| M5 | DocumentGenerationPipeline 完整实现 | 🟡 中 |
| M6 | 需求变更管理：record_requirement_change 工具 + STALE 标记 | 🟡 中 |
| M7 | 需求变更冲突检测（概念重复、规则矛盾） | 🟡 中 |
| M8 | 前端阶段进度条 + 概念面板 + 快捷建议 | 🟢 低 |
| M9 | 前端 STALE 文档提示 + 需求变更时间轴 | 🟢 低 |
| M10 | 斜线命令解析（含 `/change`, `/changes`, `/regenerate`） | 🟢 低 |
| M11 | `validate_ddd_model` 工具（DDD 合规检查） | 🟢 低 |
