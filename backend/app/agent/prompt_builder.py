...
# ---------------------------------------------------------------------------
_XML_EXTRACTION_FORMAT = """【结构化提取规则 — 强制执行】
⚠️ 以下 XML 标记是系统记录信息的唯一方式。不输出 XML 标记 = 信息不会出现在阶段文档中。
每次回复末尾，只要发生了以下任一情况，就必须输出对应的 XML 标记：

识别到业务场景时（必须立即标记，不得延迟）：
<scenario id="S001" name="场景名称">场景描述</scenario>

向用户提出澄清问题时（每个问题必须同时输出，不得仅以普通文本提问）：
<clarification id="Q001">澄清问题文本</clarification>
...
【XML 标记要求（必须执行）】
- 识别到项目名称或领域背景时，**必须**立即输出 <project_info> 标记
- 向用户提出任何问题时，**必须**同时输出 <clarification> 标记""",
...
【XML 标记要求（必须执行）】
- 确认或讨论一个业务场景时，**必须**立即输出 <scenario> 标记，不得遗漏
- 向用户提出任何澄清问题时，**必须**同时将每个问题输出为 <clarification> 标记（不能仅以普通文本提问）
- 用户回答了某个澄清问题后，**必须**输出 <clarification id="Qxxx" answered="true"/> 将其从队列删除""",
...
【XML 标记要求（必须执行）】
- 识别或确认一个领域概念时，**必须**立即输出 <concept> 标记
- 向用户提出任何澄清问题时，**必须**同时将每个问题输出为 <clarification> 标记
- 用户回答了某个澄清问题后，**必须**输出 <clarification id="Qxxx" answered="true"/> 将其删除
